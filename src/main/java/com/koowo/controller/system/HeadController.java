...
import com.koowo.service.system.AppuserService;
...
public class HeadController extends BaseController {
	...
	public ModelAndView saveSys() throws Exception{
		...
		Tools.writeFile(Const.EMAIL,pd.getString("SMTP")+",koowo,"+pd.getString("PORT")+",koowo,"+pd.getString("EMAIL")+",koowo,"+pd.getString("PAW"));	//写入邮件服务器配置
		Tools.writeFile(Const.SMS1,pd.getString("SMSU1")+",koowo,"+pd.getString("SMSPAW1"));	//写入短信1配置
		Tools.writeFile(Const.SMS2,pd.getString("SMSU2")+",koowo,"+pd.getString("SMSPAW2"));	//写入短信2配置
		mv.addObject("msg","OK");
	...
	public ModelAndView saveSys2() throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		Tools.writeFile(Const.FWATERM,pd.getString("isCheck1")+",koowo,"+pd.getString("fcontent")+",koowo,"+pd.getString("fontSize")+",koowo,"+pd.getString("fontX")+",koowo,"+pd.getString("fontY"));	//文字水印配置
		Tools.writeFile(Const.IWATERM,pd.getString("isCheck2")+",koowo,"+pd.getString("imgUrl")+",koowo,"+pd.getString("imgX")+",koowo,"+pd.getString("imgY"));	//图片水印配置
		mv.addObject("msg","OK");
		mv.setViewName("save_result");
		return mv;
	}
