package com.koowo.plugin.websocketonline;
...
import com.koowo.constant.WebSocketConst;
...
public class OnlineChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
        ...
        if (null != message && message.startsWith(WebSocketConst.JOIN)) {
            this.userjoin(message.replaceFirst("\\[join\\]", ""),conn);
        }
        if (null != message && message.startsWith(WebSocketConst.GOOUT)) {
            this.goOut(message.replaceFirst("\\[goOut\\]", ""));
        } else if (null != message && message.startsWith(WebSocketConst.LEAVE)) {
            this.userLeave(conn);
        } else if (null != message && message.startsWith(WebSocketConst.COUNT)) {
            this.getUserCount(conn);
        } else if (null != message && message.startsWith(WebSocketConst.GETUSERLIST)) {
